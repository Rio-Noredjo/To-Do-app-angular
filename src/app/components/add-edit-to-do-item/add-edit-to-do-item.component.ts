...
export class AddEditToDoItemComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.isAddMode) {
      this.addEditItemService
        .addItem(itemCategory, Number(sessionStorage.getItem('userId')))
        .subscribe({
          next: (response) => {
            this.ngOnInit();
  ...
  get getTitle() {
    return this.addEditItemFormGroup.get('title');
  }
  get getContent() {
    return this.addEditItemFormGroup.get('content');
  }
  get getStatus() {
    return this.addEditItemFormGroup.get('status');
  }
  get getCategory() {
    return this.addEditItemFormGroup.get('category');
  }
}
